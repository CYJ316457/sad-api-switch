...
pub struct ApiEntry {
    ...
    /// Downstream-facing model id. This is what clients request.
    pub model: String,
    /// Upstream-facing model id. Defaults to `model` for unmapped entries.
    #[serde(default)]
...
fn row_to_entry(row: &rusqlite::Row<'_>, include_channel: bool) -> rusqlite::Result<ApiEntry> {
    ...
    Ok(ApiEntry {
        ...
        model: row.get(2)?,
        upstream_model: empty_to_none(upstream_model),
...
impl Database {
    ...
    pub fn create_entry(
        ...
        Ok(ApiEntry {
            ...
            upstream_model: empty_to_none(upstream_model.to_string()),
        ...
        for model in selected_models {
            ...
            if !current_models.contains(model) {
                ...
                conn.execute(
                    ...
                    provider_logo, release_date, model_meta_zh, model_meta_en, group_name,
                    created_at, updated_at
                ) VALUES (?1, ?2, ?3, ?3, ?10, ?4, 1, ?5, ?6, ?7, ?8, 'auto', ?9, ?9)",
                    rusqlite::params![
                        id,
                        channel_id,
                        model,
