...
fn ensure_api_entry_columns(conn: &Connection) -> Result<(), AppError> {
    ...
    ensure_column(conn, "api_entries", "upstream_model", "TEXT DEFAULT ''")?;
    conn.execute(
        "UPDATE api_entries SET upstream_model = model WHERE upstream_model IS NULL OR TRIM(upstream_model) = ''",
        [],
    )
    .map_err(|e| AppError::Database(e.to_string()))?;
    // group_name 分组字段
