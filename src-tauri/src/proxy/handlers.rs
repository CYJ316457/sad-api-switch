...
fn gemini_single_model_item(entry: &crate::database::ApiEntry) -> Value {
    ...
    json!({
        ...
        "displayName": downstream_display_name(entry),
...
fn openai_model_item(entry: &crate::database::ApiEntry) -> Value {
    json!({
        ...
        "display_name": downstream_display_name(entry),
        "group_name": entry.group_name,
    })
}
...
fn azure_deployment_item(entry: &crate::database::ApiEntry) -> Value {
    json!({
        ...
        "created": entry_created_at(entry),
        "owned_by": entry_owned_by(entry, "openai"),
        "model": downstream_display_name(entry),
        "display_name": downstream_display_name(entry),
        "group_name": entry.group_name,
    })
}
...
mod tests {
    ...
    fn openai_model_item_includes_created_and_owned_by() {
        ...
        assert_eq!(value["display_name"], "gpt-4o");
    }
    ...
    fn claude_model_item_uses_rfc3339_created_at() {
        ...
        assert_eq!(value["display_name"], "claude-3-5-sonnet");
    ...
    fn azure_deployment_item_uses_deployment_object_and_model_fallback() {
        ...
        assert_eq!(value["display_name"], "deployment-1");
    }

    #[test]
    fn mapped_model_items_expose_downstream_names() {
        let mut entry = sample_entry("1", "client-alias", "Upstream Label", None);
        entry.upstream_model = Some("real-upstream-model".to_string());

        let openai = openai_model_item(&entry);
        let claude = claude_model_item(&entry);
        let gemini = gemini_model_item(&entry);
        let azure = azure_deployment_item(&entry);

        assert_eq!(openai["display_name"], "client-alias");
        assert_eq!(claude["display_name"], "client-alias");
        assert_eq!(gemini["displayName"], "client-alias");
        assert_eq!(azure["model"], "client-alias");
        assert_eq!(azure["display_name"], "client-alias");
    }
