...
async fn forward_single(
    ...
    let upstream_model = entry.upstream_model();
    let url = adapter.build_chat_url(&channel.base_url, upstream_model);
    ...
    if is_stream {
        ...
        let usage_tokens = extract_usage_tokens(&response_body);
...
fn usage_i64_at<'a>(usage: Option<&'a Value>, path: &[&str]) -> i64 {
    let mut current = usage;
    for key in path {
        current = current.and_then(|value| value.get(*key));
    }
    current.and_then(Value::as_i64).unwrap_or(0)
}

fn extract_usage_tokens(body: &Value) -> UsageTokens {
    let usage = body.get("usage");
    let prompt_tokens = usage_i64_at(usage, &["prompt_tokens"]);
    let completion_tokens = usage_i64_at(usage, &["completion_tokens"]);
        ...
        futures::stream::poll_fn(move |cx| -> Poll<Option<Result<Bytes, std::io::Error>>> {
            ...
            match upstream_stream.as_mut().poll_next(cx) {
                Poll::Ready(Some(Ok(chunk))) => {
                    ...
                    if needs_transform {
                        if let Some(transformed) = transform_sse_chunk_with_cache(
                            &chunk,
                            &mut sse_buffer,
                        ...
                        if let Some(with_model_info) = append_and_parse_sse_with_cache(
                            &mut sse_buffer,
                            &mut sse_utf8_remainder,
                            &chunk,
                            &prompt_tokens,
...
fn transform_sse_chunk(
    ...
    transform_sse_chunk_with_cache(
        chunk,
        buffer,
        remainder,
        adapter,
...
fn transform_sse_chunk_with_cache(
    chunk: &Bytes,
    buffer: &mut String,
    remainder: &mut Vec<u8>,
    adapter: &Box<dyn super::protocol::ProtocolAdapter + Send + Sync>,
        ...
        if let Some(payload) = line.strip_prefix("data: ") {
            ...
            if let Some(transformed) = adapter.transform_sse_line(payload) {
                if let Ok(value) = serde_json::from_str::<Value>(&transformed) {
                    let usage_tokens = extract_usage_tokens(&value);
                    if usage_tokens.cache_read_tokens > 0 {
...
fn append_and_parse_sse(
    ...
    append_and_parse_sse_with_cache(
        buffer,
        remainder,
        chunk,
        prompt_tokens,
...
fn append_and_parse_sse_with_cache(
    buffer: &mut String,
    remainder: &mut Vec<u8>,
    chunk: &Bytes,
    prompt_tokens: &Arc<AtomicI64>,
    ...
    while let Some(line_end) = buffer.find('\n') {
        ...
        if let Some(payload) = line.strip_prefix("data: ") {
            ...
            let usage_tokens = extract_usage_tokens(&value);
            if usage_tokens.prompt_tokens > 0 {
                prompt_tokens.store(usage_tokens.prompt_tokens, Ordering::Relaxed);
            }
            if usage_tokens.completion_tokens > 0 {
                completion_tokens.store(usage_tokens.completion_tokens, Ordering::Relaxed);
            }
            if usage_tokens.cache_read_tokens > 0 {
                cache_read_tokens.store(usage_tokens.cache_read_tokens, Ordering::Relaxed);
            }
