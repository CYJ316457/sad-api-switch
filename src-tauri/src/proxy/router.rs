...
mod tests {
    ...
    fn mapped_entry(
        ...
        let mut e = entry_with_group(id, downstream_model, enabled, sort_index, group);
        e.upstream_model = Some(upstream_model.to_string());
        e
    }
    ...
    #[tokio::test]
    async fn mapped_entry_routes_by_downstream_model_and_preserves_upstream_model() {
        let breakers = RwLock::new(HashMap::new());
        let all = vec![mapped_entry(
            "mapped",
            "client-model",
            "provider-real-model",
            true,
    ...
    #[test]
    fn empty_upstream_model_falls_back_to_downstream_model() {
        let mut entry = entry("alias-model", "alias-model", true, 0);
        entry.upstream_model = Some(String::new());

        assert_eq!(entry.upstream_model(), "alias-model");
    }
