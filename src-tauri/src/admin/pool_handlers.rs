...
#[derive(Deserialize)]
pub struct UpdateUpstreamModelParams {
    pub upstream_model: Option<String>,
}
...
/// PUT /admin/pool/:id/model - Update the downstream model id for an entry.
pub async fn update_model(
    State(state): State<AdminState>,
    Path(id): Path<String>,
    Json(payload): Json<UpdateModelParams>,
) -> Result<Json<serde_json::Value>, AdminError> {
...
/// PUT /admin/pool/:id/upstream-model - Update the upstream model mapping for an entry.
pub async fn update_upstream_model(
    State(state): State<AdminState>,
    Path(id): Path<String>,
    Json(payload): Json<UpdateUpstreamModelParams>,
) -> Result<Json<serde_json::Value>, AdminError> {
    let upstream_model = payload.upstream_model.unwrap_or_default();
    pool_service::update_entry_upstream_model(&state.db, &id, &upstream_model)?;
    Ok(Json(serde_json::json!({"ok": true})))
}
