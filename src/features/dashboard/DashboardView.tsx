...
export function DashboardView() {
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: () => adapter.usage.getDashboardStats(filter),
  });

  const { data: consumption } = useQuery({
    queryKey: ["modelConsumption", filter],
    queryFn: () => adapter.usage.getModelConsumption(filter),
  });

  const { data: callTrend } = useQuery({
    queryKey: ["callTrend", filter],
    queryFn: () => adapter.usage.getCallTrend(filter),
  });

  const { data: distribution } = useQuery({
    queryKey: ["modelDistribution", filter],
    queryFn: () => adapter.usage.getModelDistribution(filter),
  });

  const { data: userTrend } = useQuery({
    queryKey: ["userTrend", filter],
    queryFn: () => adapter.usage.getUserTrend(filter),
  });
