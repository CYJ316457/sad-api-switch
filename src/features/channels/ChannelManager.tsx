...
import type { ApiEntry, PaginatedResult } from '@/types';
...
function getEntryUpstreamModel(entry: Pick<ApiEntry, "model" | "upstream_model">): string {
  return entry.upstream_model?.trim() || entry.model.trim();
}
...
function parseChannelBackupPayload(input: unknown): ParsedChannelBackup {
  if (Array.isArray(input)) return { channels: input as Channel[], apiEntries: [] };
  if (input && typeof input === "object") {
    const payload = input as Partial<ChannelBackupPayload> & { channels?: unknown };
    if (Array.isArray(payload.channels)) {
      return {
        channels: payload.channels as Channel[],
        apiEntries: Array.isArray(payload.api_entries) ? payload.api_entries as ApiEntry[] : [],
      };
    }
  }
  throw new Error("Invalid channel backup file");
}
...
export const ChannelManager: React.FC = () => {
  ...
  const handleExportChannels = async () => {
    try {
      const allChannels = await api.channels.list();
      const channelIds = new Set(allChannels.map((channel) => channel.id));
      const allEntries = await api.pool.list();
      const payload: ChannelBackupPayload = {
        version: 2,
        exported_at: new Date().toISOString(),
        channels: allChannels,
        api_entries: allEntries.filter((entry) => channelIds.has(entry.channel_id)),
      };
      const stamp = new Date().toISOString().replace(/[:.]/g, "-");
      await saveTextFile(`api-switch-channels-${stamp}.json`, JSON.stringify(payload, null, 2));
    } catch (err) {
      if (err instanceof DOMException && err.name === "AbortError") return;
      toast.error(`导出渠道失败: ${err instanceof Error ? err.message : String(err)}`);
    }
  };

  const importChannelBackup = async (file: File) => {
    setImporting(true);
    try {
      const text = await file.text();
      const { channels: importedChannels, apiEntries: importedEntries } = parseChannelBackupPayload(JSON.parse(text));
      const importedEntriesByChannel = new Map<string, ApiEntry[]>();
      for (const entry of importedEntries) {
        if (!entry?.channel_id || !entry.model) continue;
        const entriesForChannel = importedEntriesByChannel.get(entry.channel_id) ?? [];
        entriesForChannel.push(entry);
        importedEntriesByChannel.set(entry.channel_id, entriesForChannel);
      }
      const existing = await api.channels.list();
      const existingIds = new Set(existing.map((channel) => channel.id));
      const preImportEntries = await api.pool.list();
      const preImportEntriesByChannel = new Map<string, ApiEntry[]>();
      for (const entry of preImportEntries) {
        const entriesForChannel = preImportEntriesByChannel.get(entry.channel_id) ?? [];
        entriesForChannel.push(entry);
        preImportEntriesByChannel.set(entry.channel_id, entriesForChannel);
      }
      const entriesToRestore: ApiEntry[] = [];

      for (const channel of importedChannels) {
        if (!channel?.id || !channel.name || !channel.api_type || !channel.base_url || !channel.api_key) {
          continue;
        }
        if (existingIds.has(channel.id)) {
          ...
          if (channel.enabled === false) {
            await api.channels.update({ id: channel.id, enabled: false });
          }
        }

        await api.channels.updateResponseMs(channel.id, channel.response_ms ?? "");
        const backupEntries = importedEntriesByChannel.get(channel.id) ?? preImportEntriesByChannel.get(channel.id) ?? [];
        entriesToRestore.push(...backupEntries);
        const selectedModelNames = backupEntries.length > 0
          ? uniqueStrings(backupEntries.map(getEntryUpstreamModel))
          : channel.selected_models ?? [];
        const availableModelNames = new Set((channel.available_models ?? []).map((model) => model.name));
        const availableModels = [...(channel.available_models ?? [])];
        for (const modelName of selectedModelNames) {
          if (availableModelNames.has(modelName)) continue;
          availableModels.push({ id: modelName, name: modelName });
          availableModelNames.add(modelName);
        }
        await api.channels.selectModels(
          channel.id,
          selectedModelNames,
          availableModels,
          selectedModelNames.map(buildEntryCatalogMeta),
        );
      }

      if (entriesToRestore.length > 0) {
        const currentEntries = await api.pool.list();
        const currentEntriesByChannelAndUpstream = new Map<string, ApiEntry[]>();
        for (const entry of currentEntries) {
          const key = `${entry.channel_id}\u0000${getEntryUpstreamModel(entry)}`;
          const matchingEntries = currentEntriesByChannelAndUpstream.get(key) ?? [];
          matchingEntries.push(entry);
          currentEntriesByChannelAndUpstream.set(key, matchingEntries);
        }
        ...
        for (const backupEntry of entriesToRestore) {
          ...
          const currentEntry = matchingEntries.shift() ?? await api.pool.create({
            channelId: backupEntry.channel_id,
            model: backupEntry.model,
            upstreamModel,
            displayName: backupEntry.display_name,
            groupName: backupEntry.group_name || "auto",
          });
          currentEntriesByChannelAndUpstream.set(key, matchingEntries);

          if (currentEntry.upstream_model !== upstreamModel) {
            await api.pool.updateUpstreamModel(currentEntry.id, upstreamModel);
          }
          if (currentEntry.model !== backupEntry.model) {
            await api.pool.updateModel(currentEntry.id, backupEntry.model);
          }
          if ((currentEntry.group_name || "auto") !== (backupEntry.group_name || "auto")) {
            await api.pool.updateGroup(currentEntry.id, backupEntry.group_name || "auto");
          }
      ...
      toast.error(`导入渠道失败: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
      setImporting(false);
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={(event) => {
                ...
                if (file) void importChannelBackup(file);
              }}
            />
